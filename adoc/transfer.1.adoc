...
endpoint. Mutually exclusive with --recursive.
...
+
*keep* will create symlinks on the destination.
+
...
Transfer the symlink at /path/to/symlink as a symlink rather than transferring
the contents of the directory it links to.
...
Recursively transfer the contents of the directory linked to by the symlink
